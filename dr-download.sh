...
for slug in "$@"
do
	data=$(curl -Ls "http://www.dr.dk/mu/programcard?Slug=%22$slug%22" | jq -r "{uri: .Data[0].PrimaryAssetUri, title: .Data[0].Title, subtitle: .Data[0].Subtitle}") 
	...
	./hls-fetch/hls-fetch --playlist ${streamUrl} -o "$slug.mp4"
	printf "\n"
done